...
impl Provider for ReliableProvider {
    ...
    async fn chat_with_system(
        ...
        let is_mostly_rate_limited = failures.iter().any(|f| f.contains("rate_limited"));
        let error_msg = if is_mostly_rate_limited {
            format!(
                "All providers/models failed due to rate limiting or transient errors. \
                 Please wait a few minutes before retrying, or check if your API account has sufficient credits/quota. \
                 Attempts:\n{}",
                failures.join("\n")
            )
        } else {
